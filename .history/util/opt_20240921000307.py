...
def ListCheck(self,context, structure, idx, dic_to_find:dict, bObj):
    scene = context.scene
    try: 
        item = structure[idx]
    except IndexError:
        ...
        if not item.Object:
            ...
            if bObj:
                if not scene.objects.find(item.Object.name) + 1:
                    structure.remove(idx)
                    idx -= 1
            else:
                if not bpy.data.collections.find(item.Collection.name) + 1:
                    structure.remove(idx)
                    idx -= 1


class OP_UI_ListCheck(Opt):
    bl_idname = OptIDName + "list_check"
    bl_label = "List check"
    ...
    bObj: bpy.props.BoolProperty(default=True)

    def execute(self, context):
        scene = context.scene

        Prop_Gropu = getattr(scene.P_S, self.panel_name)
        structure = getattr(Prop_Gropu, self.structure_name)
        idx = getattr(Prop_Gropu, self.idx_name)
        type = self.type_name
...
class OP_UI_List_actions(Opt):
    """Move items up and down, add and remove"""
    bl_idname = OptIDName + "list_action"
    ...
    def execute(self, context):
        ...
        if self.action == 'DOWN' and idx < len(structure) - 1:
            item_next = structure[idx+1].Object.name
            structure.move(idx, idx+1)
            idx += 1
            info = 'Item "%s" moved to position %d' % (item.Object.name, idx + 1)
            ...
            if len(structure) > 0:
                info = 'Item "%s" removed from list' % (structure[idx].Object.name)
                structure.remove(idx)
                idx -= 1
                self.report({'INFO'}, info)
            else:
            ...
            if self.bObj: 
                ...
                if not selected_objects:
                    ...
                    for stored_objects in structure:
                        objects_holder.append(stored_objects.Object)
                    ...
                    if difference:
                        for obj in difference:
                            ...
                            info = '"%s" added to list %s' % (item.Object.name, self.panel_name)
                            self.report({'INFO'}, info)
                    else:
...
Register = [OP_UI_ListCheck, OP_UI_SyncSelectedCollectionsToProp, OP_UI_List_actions, OP_UI_clearList]