...
class OP_MainAction(Opt):
        ...
        if self.bRandom:
            for video_index in range(self.StartVideoIndex, self.VideoMax+1):
                ...
                if self.action == 'RENDER':
                    ...
                    self.image_path = util_mkdir(self.base_path + '\\images\\' + self.video_name + '\\')
    ...
    def AnnotationOutputNodesGenerate(self):
        ...
        if Annotation_props.b
        if Annotation_props.bObjectsTrack2D:
            self.track_2D_path = util_mkdir(self.base_path + '\\labels_with_ids\\' + self.video_name + '\\')
        ...
        if Annotation_props.bDepth8bit:
            self.depth_8bit_path = util_mkdir(self.base_path + '\\depth_8bit\\' + self.video_name + '\\')
            self.create_annotation_depth_output_8bit()

        if Annotation_props.bDepthRaw:
            self.depth_raw_path = util_mkdir(self.base_path + '\\depth_raw\\' + self.video_name + '\\')
