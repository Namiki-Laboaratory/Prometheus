...
def AmountCheck(
    ...
    target_value = getattr(target_class, target_value_name) if bTargetSceneValue else getattr(context.scene, target_value_name)
    if bCheckMax:
...
class General(PropertyGroup):
    ...
    VideoMaxFrame = bpy.context.scene.frame_end
    ...
    StartFrameIndex: IntProperty(name='Start Frame Index', default=1, min=1, max=999999,
                                 update=lambda self, context : AmountCheck(self, 'StartFrameIndex', 'VideoMaxFrame', context, bCheckMax=True, bTargetSceneValue=True))
