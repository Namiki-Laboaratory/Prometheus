...
class AnnotationProcessor:
    def CreateOutputNode(self, path):
        output_node = self.scene.node_tree.nodes.new('CompositorNodeOutputFile')
    ...
    def __init__(
        ...
        self.path = mkdir(dir + self.name + "\\")
        self.file_name = file_name
        self.objects = objects

        self.output_node = self.CreateOutputNode(self.path)
...
class Track2DAnnotationProcessor(AnnotationProcessor):
    def __init__(
        ...
        self.path_crypto = mkdir(self.path + 'cryptomatte\\')
        self.image = []

        self.cryptomatte_node_list = []
        self.output_node, self.cryptomatte_node_list = self.CreateOutputNode(self.path_crypto)

    def CreateOutputNode(self, path):
        output_node = super().CreateOutputNode(path)

        cryptomatte_node_list = []
        ...
        for each_obj in self.objects:
            ...
            cryptomatte_node_list.append(cryptomatte)
            
            output_slot = output_node.file_slots.new(each_obj.name + '_')  # Only work with .file_slots or .layer_slots, .inputs not working
            self.scene.node_tree.links.new(cryptomatte.outputs['Matte'], output_slot)
        return output_node, cryptomatte_node_list
    ...
    def PostRenderProcess(self):
        ...
        with open(self.path + self.file_name + '.txt', mode='w') as trak_file:
...
class Depth8bitAnnotationProcessor(AnnotationProcessor):
    def CreateOutputNode(self, path):
        ...
        output_slot = output_node.file_slots.new('depth_8bit')

        self.view.use_pass_z = True

        compositornode_renderlayer = self.node_tree.nodes.new(type='CompositorNodeRLayers')
        compositornode_renderlayer.scene = self.scene
        compositornode_renderlayer.layer = self.view.name

        normalize_node = self.scene.node_tree.nodes.new(type='CompositorNodeNormalize')

        self.scene.node_tree.links.new(compositornode_renderlayer.outputs['Depth'], normalize_node.inputs[0])
        self.scene.node_tree.links.new(compositornode_renderlayer.outputs['Depth'], output_slot)
...
class OP_MainAction(Opt):
    ...
    def execute(self, context):
        ...
        if self.bRandom:
            for video_index in range(self.StartVideoIndex, self.VideoMax+1):
                ...
                # if self.base_path_origin[0:4] == '/tmp':
                #     self.base_path = r'C:\tmp' + self.base_path_origin[5:]
                # else:
                ...
                if self.action == 'RENDER':
                    ...
                    self.image_path = mkdir(self.base_path + '\\images\\' + self.video_name + '\\')
    ...
    def create_annotation_depth_output_8bit(self):
        ...
        output_node.format.compression = 0
        output_node.base_path = self.depth_8bit_path

        self.OutputNodes['depth_8bit'] = output_node

        normalize_node = self.node_tree.nodes.new(type='CompositorNodeNormalize')

        links = self.node_tree.links
        link = links.new(self.compositornode_renderlayer.outputs['Depth'], normalize_node.inputs[0])
        link = links.new(normalize_node.outputs[0], output_node.inputs['Image'])

    def create_annotation_depth_output_raw(self):
        output_node = self.node_tree.nodes.new(type='CompositorNodeOutputFile')
        output_node.format.file_format = 'OPEN_EXR'
        output_node.format.color_depth = '32'
        output_node.format.compression = 0
        output_node.base_path = self.depth_raw_path

        self.OutputNodes['depth_raw'] = output_node

        links = self.node_tree.links
        link = links.new(self.compositornode_renderlayer.outputs['Depth'], output_node.inputs['Image'])
    ...
    def AnnotationOutputNodesGenerate(self):
        ...
        if Annotation_props.bTracking:
            if Annotation_props.bObjectsTrack2D:
                self.track_2D_path = mkdir(self.base_path + '\\labels_with_ids\\' + self.video_name + '\\')
                self.write_id_into_json(self.class_list_source, self.Objects, (self.base_path + '\\labels_with_ids\\' + self.video_name + '\\'))

        # Depth
        if Annotation_props.bDepth:
            if Annotation_props.bDepth8bit:
                self.depth_8bit_path = mkdir(self.base_path + '\\depth_8bit\\' + self.video_name + '\\')
                self.create_annotation_depth_output_8bit()

            if Annotation_props.bDepthRaw:
                self.depth_raw_path = mkdir(self.base_path + '\\depth_raw\\' + self.video_name + '\\')
                self.create_annotation_depth_output_raw()
    ...
    def RenderAction(self):
        ...
        for i_camera, each_camera in enumerate(self.Cameras):
            ...
            for i in range(self.StartFrameIndex, scene.frame_end + 1):
                scene.frame_set(i)
                image_name = str(i_camera).zfill(4) + '_' + 4*'#'
                # scene.render.filepath = self.image_path  # Set save path with frame

                for each_outputnode in self.OutputNodes.values():
                    each_outputnode.file_slots.values()[0].path = image_name

                if self.Props.General.bObjectsTrack2D:
                    Output_cryptmatte = Track2DAnnotationProcessor(
                        self.track_2D_path,
                        self.scene_track, 
                        self.ViewLayer, 
                        self.Objects
                ...
                if self.Props.General.bObjectsTrack2D:
                    Output_cryptmatte.Find_bbox(image_name, self.track_2D_path)
