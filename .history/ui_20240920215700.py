...
class P_Annotation_Track(P_Master):
    bl_idname = P_Annotation.bl_idname + 'Track'
    bl_label = 'Track'
    bl_parent_id = P_Annotation.bl_idname

    def draw_header(self, context: Context):
        super().draw_header(context)
        self.layout.prop(self.props_annotation, 'bTracking')

    def draw(self, context):
        super().draw(context)
        self.layout.prop(self.props_annotation, 'bTracking2D')


AllPanelClasses = [P_General, P_Object, P_Material, P_Light, P_Camera, P_Physics, P_Movement, P_Annotation]
