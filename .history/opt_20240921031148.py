...
class OP_MainAction(Opt):
    ...
    def execute(self, context):
        ...
        if self.bRandom:
            for video_index in range(self.StartVideoIndex, self.VideoMax+1):
                ...
                video_name = str(video_index).zfill(4)
                ...
                if self.action == 'RENDER':
                    ...
                    self.image_path = util_mkdir(self.base_path + '\\images\\' + video_name + '\\')

                    if General_props.bObjectsTrack2D:
                        self.track_2D_path = util_mkdir(self.base_path + '\\labels_with_ids\\' + video_name + '\\')
                        self.write_id_into_json(self.class_list_source, self.Objects, (self.base_path + '\\labels_with_ids\\' + video_name + '\\'))

                    if General_props.bDepth8bit:
                        self.depth_8bit_path = util_mkdir(self.base_path + '\\depth_8bit\\' + video_name + '\\')
                        self.create_annotation_depth_output_8bit()

                    if General_props.bDepthRaw:
                        self.depth_raw_path = util_mkdir(self.base_path + '\\depth_raw\\' + video_name + '\\')
    ...
    def AnnotationOutputNodesGeneration(self):
