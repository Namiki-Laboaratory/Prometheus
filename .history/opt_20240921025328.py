...
class OP_MainAction(Opt):
            ...
            for video_num in range(self.StartVideoIndex, self.VideoMax+1):
                ...
                if self.action == 'RENDER':
                    ...
                    if General.bObjectsTrack2D:
                        self.track_2D_path = util_mkdir(self.path + '\\labels_with_ids\\' + video_name + '\\')
                        self.write_id_into_json(self.class_list_source, self.Objects, (self.path + '\\labels_with_ids\\' + video_name + '\\'))

                    if General.bDepth8bit:
                        self.depth_8bit_path = util_mkdir(self.path + '\\depth_8bit\\' + video_name + '\\')
                        self.create_annotation_depth_output_8bit()

                    if General.bDepthRaw:
                        self.depth_raw_path = util_mkdir(self.path + '\\depth_raw\\' + video_name + '\\')
    ...
    def create_scene(self) -> Tuple[bpy.types.Scene, bpy.types.]:
        ...
        ViewLayer = self.scene_track.view_layers.new(layer_name)
