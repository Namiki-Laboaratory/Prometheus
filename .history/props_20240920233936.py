...
class General(PropertyGroup):
    ...
    StartVideoIndex: IntProperty(name='Start Video Index', default=1, min=1, max=9999,
                                 update=lambda self, context : AmountCheck(self, context, 'StartVideoIndex', 'VideoNumber', bCheckMax=True))
    StartFrameIndex: IntProperty(name='Start Frame Index', default=1, min=1, max=999999,
                                 update=lambda self, context : AmountCheck(self, context, 'StartFrameIndex', 'frame_end', bCheckMax=True, bTargetSceneValue=True))
    HDRIRoot: StringProperty(name='HDRI Background path', default='/tmp\\', subtype='DIR_PATH')

    VideoMaxFrame = IntProperty(default=context.scene.frame_end)
    try:
        VideoMaxFrame = context.scene.frame_end
    except:
        pass
...
class Objects(PropertyGroup): 
    ...
    MaxAmount: IntProperty(name="Max", default=1, min=1, max=1024,
                           update=lambda self, context: AmountCheck(self, context, 'MaxAmount', 'MinAmount', bCheckMax=False))
    MinAmount: IntProperty(name="Min", default=1, min=1, max=1024,
                           update=lambda self, context : AmountCheck(self, context, 'MinAmount', 'MaxAmount', bCheckMax=True))
...
class MaterialMeshListStructure(BaseUIListStructure):
    ...
    MetallicMax: FloatProperty(name='Metallic Max', default=1, min=0, max=1,
                                update= lambda self, context : AmountCheck(self, context, 'MetallicMax', 'MetallicMin', bCheckMax=False))
    MetallicMin: FloatProperty(name='Metallic Min', default=0, min=0, max=1,
                                update= lambda self, context : AmountCheck(self, context, 'MetallicMin', 'MetallicMax', bCheckMax=True))
    RoughnessMax: FloatProperty(name='Roughness Max', default=1, min=0, max=1,
                                 update= lambda self, context : AmountCheck(self, context, 'RoughnessMax', 'RoughnessMin', bCheckMax=False))
    RoughnessMin: FloatProperty(name='Roughness Min', default=0.4, min=0, max=1,
                                 update= lambda self, context : AmountCheck(self, context, 'RoughnessMin', 'RoughnessMax', bCheckMax=True))
    SpecularMax: FloatProperty(name='Specular Max', default=1, min=0, max=1,
                                update= lambda self, context : AmountCheck(self, context, 'SpecularMax', 'SpecularMin', bCheckMax=False))
    SpecularMin: FloatProperty(name='Specular Min', default=0.5, min=0, max=1,
                                update= lambda self, context : AmountCheck(self, context, 'SpecularMin', 'SpecularMax', bCheckMax=True))
...
class Movement(PropertyGroup):
    ...
    MaxVel: FloatProperty(name="Max Velocity", default=1, min=1, max=1024, 
                          update=lambda self, context: AmountCheck(self, context, 'MaxVel', 'MinVel', bCheckMax=False), unit='VELOCITY')
